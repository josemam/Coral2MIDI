...
bool Write(const char* name, const char* data, unsigned int chpos, unsigned char instrument, unsigned char tempo)
{
  ...
  if (f)
  {
	...
	unsigned char tempo_h = tempo >> 5;
	unsigned char tempo_l = tempo << 3;
    char header[52] = {0x4d, 0x54, 0x68, 0x64, 0x00, 0x00, 0x00, 0x06, 0x00, 0x01, 0x00, 0x02, tempo_h, tempo_l, 0x4d, 0x54, 0x72, 0x6b, 0x00, 0x00, 0x00, 0x1a, 0x00, 0xff, 0x58, 0x04, 0x04, 0x02, 0x18, 0x08, 0x00, 0xff, 0x59, 0x02, 0x00, 0x00, 0x00, 0xff, 0x51, 0x03, 0x07, 0xa1, 0x20, 0xa0, 0x00, 0xff, 0x2f, 0x00, 0x4d, 0x54, 0x72, 0x6b};
    f.write(header, 52);
...
bool Write(const char* name, const Voice v[], int INPUT_LIMIT, unsigned char instrument, unsigned char tempo)
{
  char data[INPUT_LIMIT];
  unsigned int chpos;
  return GetChunk(v, data, chpos, INPUT_LIMIT) && Write(name, data, chpos, instrument, tempo);
}
